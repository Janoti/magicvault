...
export default function SharedWithMePage() {
  ...
  return (
    <div className="p-6 max-w-3xl">
      <div className="mb-6">
        <h1 className="font-display text-3xl font-bold text-vault-gold">{t('shared.title')}</h1>
        <p className="text-vault-muted text-sm mt-0.5">{t('shared.subtitle')}</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 surface">
          ...
          <p className="text-vault-muted">{t('shared.empty')}</p>
        </div>
      ) : (
            ...
            return (
              <button key={s.id} onClick={() => setOpenId(s.id)}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-vault-muted">{t('shared.by', { name: s.owner?.username })}</p>
                </div>
